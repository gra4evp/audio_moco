...
class SiameseNet(torch.nn.Module):
    def __init__(self, base_encoder, dim=16, mlp_dim=2048, T=1.0):
        ...
        del self.encoder.fc # remove original fc layer
    ...
    def contrastive_loss(self, q1, q2):
        q1 = nn.functional.normalize(q1, dim=1)
        q2 = nn.functional.normalize(q2, dim=1)
        logits = torch.einsum('nc,mc->nm', [q1, q2]) / self.T
        N = logits.shape[0]
...
def make_base_encoder(model, pretrained=True):
    @functools.wraps(model)
    def wrapper(*args, **kwargs):
        # Создаем экземпляр модели
        encoder = model(*args, **kwargs, pretrained=pretrained)
        return encoder
    return wrapper
